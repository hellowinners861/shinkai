...
- Status: Ultra承認済み（モバイルファースト／100種対応／二本スティック式サーチライト／深度4章）
...
サーチライト識別、深度4章、カテゴリ別行動、大型生物イベント（active中の大型個体への識別フォーカス優先を含む）の最新正本は `docs/SEARCHLIGHT_CHAPTERS_V3.md` とする。同範囲で既存文書と競合する場合はV3を優先し、危険ウェーブ、得点内訳、結果、自己ベストは `docs/GAMEPLAY_LOOP_V2.md` を引き続き正本とする。
