...
- 各生物はピクセルアイコンと、照射対象または残存進捗がある間だけ真上に表示する固定サイズの横識別ゲージで表現する。種ごとの円形ハロー／リング、名前ラベル、照準線は描画しない。ゲージの正本は `docs/CREATURE_SCAN_GAUGE_V5.md` とし、既存HTMLレールは読み上げ・補助表示として維持する。全体ソナーリングとピクセルアイコンの描画は維持する。
...
- 失敗条件: 燃料が0になる
- 生物は260px・半角18度のサーチライト円錐内で選択され、レア度別の頭上識別ゲージが満タンになった時点で初めて「発見」と「観測データ獲得」を同時に成立させ、図鑑へ保存する。物理接触や旧96px近距離だけでは獲得しない。
