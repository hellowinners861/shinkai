...
### 10.1.1 潜航中の開発専用speciesアイコン

- 正式素材が揃うまで、潜航中のspecies表示はPhaser `Graphics`の塗り矩形で描く開発専用アイコンとする。アイコンは正式な写真・科学イラスト・リリース素材ではない。
- 既存の24定義（8 families × 3 variants）は、未登録IDに対する汎用フォールバックとして保持する。
- canonical catalog species 149種すべてに `source_catalog_id` 固有の色付き図形を割り当てる。内訳は fish 57、gelatinous 20、squid 16、octopus 16、crab 16、shrimp 16、other invertebrate 8 とし、固有定義を優先する。
- 未知IDだけが決定的なcategory/familyフォールバックを使い、`large_creature`は通常の汎用割当に使わない。
- 潜航中のアイコンにspecies名は表示しない。獲得、図鑑キー、スコア、出現、当たり判定の正本は既存のspeciesデータとロジックを維持する。
- 既存のrelease承認写真4点、`speciesAssets`、asset manifest、図鑑画像は変更せず、正式素材として保持する。
- 図鑑の発見済みカードでは、release承認済みのローカル写真が存在する場合は写真を優先し、カード内に作者、ライセンス（ライセンスURLリンク）、出典ページの簡潔な写真クレジットを表示する。存在しない場合は `source_catalog_id` に対応するspecies固有アイコンをHTML Canvasへ描画し、写真クレジットは表示しない。
- このアイコンは写真または引用の代替表示であり、写真／引用そのものではない。既存の説明、学名、深度、獲得数、分類・深度・事実の出典リンクは維持する。
...
>
> Terraは本プロジェクトでは使用しない。調査および採用判断の設計・レビューはUltraが担当し、検証・変換・統合はLuna Maxが担当する。
>
