...
- Status: Ultra承認済み（モバイルファースト／100種対応／二本スティック式サーチライト／深度4章／調査艇・照射灯V4）
...
3. 右スティックでサーチライトを向け、光の円錐内へ深海生物を捉えて非致死的な観測データを識別・獲得する。
4. 岩や大型生物を回避する。
5. 衝突時は燃料が減る。
6. 燃料アイテムを回収して回復する。
...
- 画面下部の左へMOVE、右へLIGHTのフローティング仮想スティックを置く。各操作領域は通常128px、最小112px以上、つまみは48px以上とする。
...
- 各生物の表示コンテナはピクセルアイコンだけを含み、種ごとの円形ハロー／リングを描画しない。能動スキャンでも生物周囲の円は復活させず、艇から対象への低不透明度線とHUDレールだけを使用する。全体ソナーリングとピクセルアイコンの描画は維持する。
- 遭遇テキストは、遭遇直前の永続図鑑進捗にaccepted scientific nameが存在しない場合だけ `NEW! / <display name>` を表示する。既発見種は後続潜航の初回遭遇を含めてNEWを表示せず、発見進捗の永続化とアクセシビリティ通知は維持する。同一フレームに発見と獲得が起きた場合はNEWを表示の優先とし、獲得表示で上書きしない。既発見種の獲得では通常のスコア付き獲得表示を許可する。
- 明るさ調整は水中プレイフィールドの深度グラデーションだけに限定し、HUD、パネル、警告色、アイコン配色、コントラストの意味は変更しない。深海のムードを維持し、GameSceneは `0x0d3440`、`0x092632`、`0x041016` を基準に、config背景は `#0a2230` とする。
...
サーチライト識別、深度4章、カテゴリ別行動、大型生物イベント（active中の大型個体への識別フォーカス優先を含む）の最新正本は `docs/SEARCHLIGHT_CHAPTERS_V3.md` とする。潜水艦本体、回転式照射灯、光束の表示は `docs/SUBMARINE_SEARCHLIGHT_V4.md` を正本とする。同範囲で既存文書と競合する場合はV4、V3の順に優先し、危険ウェーブ、得点内訳、結果、自己ベストは `docs/GAMEPLAY_LOOP_V2.md` を引き続き正本とする。
...
- 失敗条件: 燃料が0になる
- 生物はライト／探知範囲に入ると「発見」、中心距離96論理px以内でレア度別の追従スキャンを完了すると「観測データ獲得」とする。物理接触だけでは獲得しない。
...
今後確認する事項は、正式名称、世界観、振動の既定、図鑑文体、個別許諾素材を採用するかである。調査艇と照射灯の初回デザインはV4で確定済みとする。
